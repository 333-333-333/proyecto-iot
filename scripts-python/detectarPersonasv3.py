...
def detection_thread():
    ...
    while True:
        ...
        if count > 0 and personas_previas == 0 and (current_time - last_event_time) > cooldown_segundos:
            ...
            print(f"{payload['timestamp']} → Evento detectado: {count} persona(s). Total acumulado: {total_personas_pasaron}")
            last_event_time = current_time

        personas_previas = count
...
def gen():
    ...
    while True:
        ...
        cv2.putText(frame, f"Personas en escena: {len(people)}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)

        cv2.putText(frame, f"Total personas detectadas: {total_personas_pasaron}", (10, 70),
